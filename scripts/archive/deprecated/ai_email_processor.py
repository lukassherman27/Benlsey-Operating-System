...
class AIEmailProcessor:
    def __init__(self):
        logger.info("Initializing AI Email Processor")
        ...
        logger.info("AI Email Processor initialized successfully")
    ...
    def process_email(self, email_id: int):
        ...
        if not email:
            logger.warning(f"Email not found: email_id={email_id}")
            return None

        logger.info(f"Processing email from {email['sender_email']}: {email['subject'][:60]}")
        ...
        if not context['projects']:
            logger.info(f"Sender not linked to projects, categorizing only: {email['sender_email']}")
        ...
        logger.info(f"Found {len(context['projects'])} related projects for email_id={email_id}")
        ...
        try:
            ...
            logger.error(f"AI analysis failed for email_id={email_id}: {e}", exc_info=True)
            raise

        if not ai_decision:
            logger.info(f"No action needed for email_id={email_id}")
        ...
        try:
            ...
            logger.info(f"Successfully applied AI decisions for email_id={email_id}")
        except Exception as e:
            logger.error(f"Failed to apply AI decisions for email_id={email_id}: {e}", exc_info=True)
            raise
...
"""
    ...
    def process_recent_emails(self, limit=50):
        ...
        # Get all uncategorized emails (not limited by date)
        self.cursor.execute("""
...
def main():
    ...
    try:
        # Process ALL uncategorized emails
        processor.process_recent_emails(limit=10000)
    finally:
        processor.close()
