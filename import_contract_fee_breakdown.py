...
EXCEL_FILE = "/Users/lukassherman/Library/CloudStorage/OneDrive-Personal/CONTRACT_FEE_BREAKDOWN.xlsx"
...
def import_invoices(conn, df):
    ...
    for idx, row in df.iterrows():
        ...
        for i in range(1, 5):
            ...
            cursor.execute("SELECT invoice_id FROM invoices WHERE invoice_number = ?", (inv_num,))
            if cursor.fetchone():
                skipped_count += 1
                errors.append(f"Row {idx}: Invoice {inv_num} already exists")
                continue
