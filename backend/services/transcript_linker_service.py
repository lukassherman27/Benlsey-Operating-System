...
Do not explain - just provide the code or UNSURE."""
    ...
    def process_single_transcript(self, transcript_id: int, use_ai: bool = True) -> Dict:
        """
        Process a single transcript and create suggestion if match found.

        Args:
            transcript_id: ID of the transcript to process
            use_ai: Whether to use AI analysis if code/name matching fails

        Returns:
            Result dict with success, match info, and suggestion_id
        """
        # Get transcript
        ...
        if transcript.get('proposal_id') or transcript.get('project_id'):
            return {
                "success": True,
                "skipped": True,
        ...
        if match:
            match['method_used'] = 'code_extraction'

        # Strategy 2: Name matching
        if not match:
            match = self.match_by_name(transcript, proposals)
            if match:
                match['method_used'] = 'name_matching'

        # Strategy 3: AI analysis (only if enabled)
        if not match and use_ai and self.use_ai:
            match = self.match_by_ai(transcript, proposals)
            if match:
                match['method_used'] = 'ai_analysis'
        ...
        if self.create_suggestion(transcript, match):
            # Get the suggestion_id we just created
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                ...
                """, (transcript_id,))
                row = cursor.fetchone()
                suggestion_id = row[0] if row else None
            ...
            return {
                ...
                "project_code": match['proposal']['project_code'],
                "project_name": match['proposal'].get('project_name'),
                "confidence": match['confidence'],
                "method": match['method_used'],
